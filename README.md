<!-- ╔══════════════════════════════════════════════════════════════════╗ -->
<!-- ║               PEDDI SAI VENKAT SUMANTH — GitHub Profile         ║ -->
<!-- ║               ✨ Crafted with precision & passion ✨             ║ -->
<!-- ╚══════════════════════════════════════════════════════════════════╝ -->

<!-- ═══════════════════════ HEADER SECTION ═══════════════════════ -->

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=cylinder&color=0:000000,50:111111,100:D4AF37&height=230&section=header&text=PEDDI%20SAI%20VENKAT%20SUMANTH&fontSize=42&fontColor=ffffff&animation=fadeIn&fontAlignY=40&desc=⚡%20Developer%20%20%7C%20%20🤖%20AI%20Builder%20%20%7C%20%20🎮%20Game%20Creator%20%20%7C%20%20🚀%20Innovator&descAlignY=65&descSize=17&descColor=FFD700&stroke=D4AF37&strokeWidth=1" width="100%"/>

<br/>

<!-- ═══ PERSONAL LOGO ═══ -->
<a href="https://github.com/saivenkatsumanth9-stack">
<img src="https://raw.githubusercontent.com/saivenkatsumanth9-stack/saivenkatsumanth9-stack/main/personal-logo.jpg" width="150" style="border-radius:50%;" alt="PSV Logo"/>
</a>

<br/><br/>

<!-- ═══ TYPING ANIMATION — Multiple lines with personality ═══ -->
<a href="https://github.com/saivenkatsumanth9-stack">
<img src="https://readme-typing-svg.demolab.com?font=Outfit&weight=700&size=28&duration=2000&pause=500&color=D4AF37&center=true&vCenter=true&multiline=true&repeat=true&width=750&height=75&lines=Hey+there!+I'm+Sumanth+%F0%9F%91%8B;Welcome+to+my+digital+universe+%F0%9F%8C%8C" alt="Typing SVG" />
</a>

<br/>

<a href="https://github.com/saivenkatsumanth9-stack">
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=18&duration=2500&pause=800&color=FFD700&center=true&vCenter=true&repeat=true&width=800&height=35&lines=🎓+3rd+Year+B.Tech+CSE+%40+NNRES;💻+Python+%7C+Java+%7C+C+%7C+C%23+Developer;🤖+AI+%26+Machine+Learning+Enthusiast;🎮+Unity+Game+Developer;⚛️+Exploring+Quantum+Computing;🚀+Building+the+future+one+commit+at+a+time;🤝+Open+to+Internships+%26+Collaborations" alt="Typing SVG" />
</a>

<br/><br/>

<!-- ═══ SOCIAL BADGES — Modern glassmorphism style ═══ -->
<a href="https://linkedin.com/in/saivenkatsumanth"><img src="https://img.shields.io/badge/LinkedIn-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=0077B5"/></a>&nbsp;
<a href="https://github.com/saivenkatsumanth9-stack"><img src="https://img.shields.io/badge/GitHub-%23181717.svg?style=for-the-badge&logo=github&logoColor=white"/></a>&nbsp;
<a href="mailto:saivenkatsumanth9@gmail.com"><img src="https://img.shields.io/badge/Gmail-%23EA4335.svg?style=for-the-badge&logo=gmail&logoColor=white"/></a>&nbsp;
<a href="tel:+917075867969"><img src="https://img.shields.io/badge/WhatsApp-%2325D366.svg?style=for-the-badge&logo=whatsapp&logoColor=white"/></a>

<br/><br/>

<!-- ═══ PROFILE METRICS BADGES ═══ -->
<img src="https://komarev.com/ghpvc/?username=saivenkatsumanth9-stack&color=D4AF37&style=for-the-badge&label=PROFILE+VIEWS"/>
&nbsp;
<img src="https://img.shields.io/github/followers/saivenkatsumanth9-stack?style=for-the-badge&color=D4AF37&labelColor=0d1117&label=FOLLOWERS&logo=github"/>
&nbsp;
<img src="https://img.shields.io/github/stars/saivenkatsumanth9-stack?style=for-the-badge&color=D4AF37&labelColor=0d1117&label=TOTAL+STARS&logo=github&affiliations=OWNER"/>

<br/><br/>

<!-- ═══ INTERACTIVE DETAILS POPUP ═══ -->
<details>
  <summary style="list-style: none; outline: none; cursor: pointer;">
    <img src="https://img.shields.io/badge/🔮_TAP_TO_POP_OPEN_SECRET_DRAWER-CLICK_HERE!-D4AF37?style=for-the-badge&logo=probot&logoColor=black&labelColor=000000" alt="Interactive Popup Button"/>
  </summary>
  <br/>
  <table width="100%">
    <tr>
      <td align="center" style="background-color: #050505; border: 2px solid #D4AF37; border-radius: 12px; padding: 20px;">
        <h3 style="color: #D4AF37; margin-top: 0;">✨ You found Peddi's Secret Drawer! ✨</h3>
        <p style="color: #FFD700; font-size: 15px;">Welcome to this interactive space! Here are some cool facts & animations:</p>
        
        <br/>
        
        <!-- Animated typing inside the popup -->
        <img src="https://readme-typing-svg.demolab.com?font=Outfit&weight=600&size=16&duration=2000&pause=500&color=D4AF37&center=true&vCenter=true&width=500&height=30&lines=💡+I+love+building+fully+autonomous+systems;⚡+I+enjoy+designing+interactive+user+experiences;👾+Currently+working+on+a+secret+Unity+game" alt="Popup Typing" />
        
        <br/><br/>
        
        <!-- Cool status badges inside the popup -->
        <img src="https://img.shields.io/badge/Status-Coding_at_Speed_of_Light-success?style=flat-square&logo=visual-studio-code&logoColor=white" />
        &nbsp;&nbsp;
        <img src="https://img.shields.io/badge/Mode-Always_Learning-D4AF37?style=flat-square&logo=git&logoColor=white" />
        &nbsp;&nbsp;
        <img src="https://img.shields.io/badge/Vibe-Coffee_Driven-orange?style=flat-square" />
      </td>
    </tr>
  </table>
</details>

</div>

<br/>

<!-- ═══ CUSTOM WAVE DIVIDER ═══ -->
<img src="./assets/wave-header.svg" width="100%" alt="wave"/>

<!-- ═══════════════════════ ABOUT ME ═══════════════════════ -->

## <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="28"/> &nbsp;About Me

<img align="right" width="300" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif"/>

```python
#!/usr/bin/env python3
# ╔══════════════════════════════════════════════════════════╗
# ║          🧬  Sai Venkat Sumanth — Decoded  🧬           ║
# ╚══════════════════════════════════════════════════════════╝

class SumanthPeddi:
    """A developer who turns caffeine into code ☕→💻"""

    def __init__(self):
        self.name       = "Peddi Sai Venkat Sumanth"
        self.alias      = "Sumanth"
        self.location   = "📍 Hyderabad, India 🇮🇳"
        self.university = "🎓 NNRES — B.Tech CSE (2023–2027)"
        self.cgpa       = 7.45

    @property
    def tech_arsenal(self) -> dict:
        return {
            "⚔️ Battle-Tested" : ["Python", "Java", "C", "C#"],
            "🌐 Web Craft"     : ["HTML", "CSS", "JavaScript"],
            "🛠️ Power Tools"   : ["Git", "VS Code", "MySQL", "Unity"],
            "🌱 Leveling Up"   : ["React", "Node.js", "TensorFlow"],
        }

    @property
    def quest_log(self) -> list:
        return [
            "🤖 Building AI systems that actually help people",
            "🎮 Crafting immersive games in Unity",
            "📊 Making data tell compelling stories",
            "⚛️  Unlocking quantum computing mysteries",
            "🌐 Architecting full-stack experiences",
        ]

    @property
    def current_status(self) -> str:
        return "🎯 Seeking Software Internships & Fresher Roles"

    def daily_routine(self) -> str:
        return "☕ Coffee → 💡 Idea → 💻 Code → 🐛 Debug → 🚀 Ship → 🔁 Repeat"

    def __str__(self):
        return "Let's build something extraordinary together! 🚀✨"

me = SumanthPeddi()
print(me)  # Let's build something extraordinary together! 🚀✨
```

<br clear="right"/>

<!-- ═══ FUN FACTS STRIP ═══ -->
<div align="center">

> 🧠 *"I don't just write code — I craft experiences, solve puzzles, and build futures."*

<table>
<tr>
<td align="center">🏆<br/><b>3+</b><br/><sub>Projects Shipped</sub></td>
<td align="center">☕<br/><b>∞</b><br/><sub>Cups of Coffee</sub></td>
<td align="center">💻<br/><b>4</b><br/><sub>Languages</sub></td>
<td align="center">🎮<br/><b>1</b><br/><sub>Game Published</sub></td>
<td align="center">🤖<br/><b>2</b><br/><sub>AI Projects</sub></td>
<td align="center">📅<br/><b>2027</b><br/><sub>Graduating</sub></td>
</tr>
</table>

</div>

---

<!-- ═══════════════════════ TECH STACK ═══════════════════════ -->

## <img src="https://media.giphy.com/media/WUlplcMpOCEmTGBtBW/giphy.gif" width="28"/> &nbsp;Tech Arsenal

<div align="center">

<!-- ═══ SCROLLING TECH MARQUEE ═══ -->
<img src="./assets/scrolling-skills.svg" width="100%" alt="Tech Marquee"/>

<br/><br/>

<!-- ═══ LANGUAGES ═══ -->
<details open>
<summary><h3>⚔️ Languages I Speak (Fluently)</h3></summary>
<br/>

<img src="https://skillicons.dev/icons?i=python,java,c,cs&theme=dark&perline=4" alt="Languages"/>

<br/><br/>

<!-- Custom Proficiency Bars -->
| Language | Proficiency | Level |
|:---------|:------------|:------|
| 🐍 Python | ![](https://img.shields.io/badge/████████████████████░░░░-85%25-D4AF37?style=flat-square&labelColor=0d1117) | ⭐⭐⭐⭐⭐ Advanced |
| ☕ Java | ![](https://img.shields.io/badge/██████████████████░░░░░░-75%25-C5A028?style=flat-square&labelColor=0d1117) | ⭐⭐⭐⭐ Intermediate+ |
| 🔧 C | ![](https://img.shields.io/badge/████████████████░░░░░░░░-65%25-B69119?style=flat-square&labelColor=0d1117) | ⭐⭐⭐⭐ Intermediate |
| 🎮 C# | ![](https://img.shields.io/badge/██████████████████░░░░░░-75%25-A7820A?style=flat-square&labelColor=0d1117) | ⭐⭐⭐⭐ Intermediate+ |

</details>

<!-- ═══ TOOLS & FRAMEWORKS ═══ -->
<details open>
<summary><h3>🛠️ Tools & Technologies</h3></summary>
<br/>

<img src="https://skillicons.dev/icons?i=git,github,vscode,mysql,unity,html,css,js&theme=dark&perline=8" alt="Tools"/>

</details>

<!-- ═══ CURRENTLY LEARNING ═══ -->
<details open>
<summary><h3>🌱 Currently Leveling Up</h3></summary>
<br/>

<img src="https://skillicons.dev/icons?i=react,nextjs,nodejs,tensorflow,pytorch&theme=dark&perline=5" alt="Learning"/>

<br/><br/>

```
📚 Learning Roadmap 2025
├── 🌐 Frontend ──── React → Next.js → Three.js
├── ⚙️ Backend ───── Node.js → Express → MongoDB
├── 🤖 AI/ML ─────── TensorFlow → PyTorch → LangChain
└── ☁️ Cloud ─────── Docker → AWS → Kubernetes
```

</details>

<!-- ═══ PLATFORMS ═══ -->
<details open>
<summary><h3>🖥️ Platforms</h3></summary>
<br/>

<img src="https://skillicons.dev/icons?i=windows,linux,androidstudio&theme=dark&perline=3" alt="Platforms"/>

</details>

</div>

---

<!-- ═══════════════════════ FEATURED PROJECTS ═══════════════════════ -->

## <img src="https://media.giphy.com/media/iY8CRBdQXODJSCERIr/giphy.gif" width="28"/> &nbsp;Featured Projects

<div align="center">

<br/>

<!-- ═══════ PROJECT 1: AI STUDY BUDDY ═══════ -->

<table>
<tr>
<td>

<div align="center">

### 🤖 AI Study Buddy
#### *Plan Smart. Study Better. Achieve More.*

<br/>

<img src="https://raw.githubusercontent.com/saivenkatsumanth9-stack/saivenkatsumanth9-stack/main/study-buddy.jpg" width="180" alt="AI Study Buddy"/>

<br/><br/>

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white&labelColor=1a1a2e)
![NLP](https://img.shields.io/badge/NLP-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white&labelColor=1a1a2e)
![RAG](https://img.shields.io/badge/RAG-D4AF37?style=for-the-badge&labelColor=1a1a2e)
![AI APIs](https://img.shields.io/badge/AI_APIs-10A37F?style=for-the-badge&logo=openai&logoColor=white&labelColor=1a1a2e)

</div>

---

| Feature | Description |
|:--------|:------------|
| 🧠 **Smart Scheduling** | AI-powered personalized study plans |
| 📄 **RAG Chat Engine** | Ask questions directly from your PDFs |
| 📝 **Auto Quiz Gen** | AI creates quizzes tailored to your syllabus |
| ⚡ **Undergrad RAG** | Advanced RAG at undergraduate level |

> 💡 *One of the few undergraduate projects implementing production-grade RAG*

</td>
</tr>
</table>

<br/>

<!-- ═══════ PROJECT 2: HEALTHMATE ═══════ -->

<table>
<tr>
<td>

<div align="center">

### 🏋️ HealthMate
#### *Your Health, Our Priority.*

<br/>

<img src="https://raw.githubusercontent.com/saivenkatsumanth9-stack/saivenkatsumanth9-stack/main/healthmate.jpg" width="180" alt="HealthMate"/>

<br/><br/>

![HTML](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white&labelColor=1a1a2e)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white&labelColor=1a1a2e)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black&labelColor=1a1a2e)
![AI](https://img.shields.io/badge/AI_Chatbot-10A37F?style=for-the-badge&logo=openai&logoColor=white&labelColor=1a1a2e)

</div>

---

| Feature | Description |
|:--------|:------------|
| 📱 **Smart Onboarding** | Responsive SPA with intelligent user flow |
| 🔢 **Health Calculators** | BMI / BMR / TDEE + food nutrition logger |
| 🏃 **Workout Plans** | Gym & home routines + visual progress charts |
| 🤖 **AI Fitness Coach** | Integrated chatbot for personalized advice |

> 💡 *Full-stack health app with real AI integration — not just a template*

</td>
</tr>
</table>

<br/>

<!-- ═══════ PROJECT 3: LAST THIEF STANDING ═══════ -->

<table>
<tr>
<td>

<div align="center">

### 🎮 Last Thief Standing
#### *Run. Loot. Survive. Be the Last. Steal it All.*

<br/>

<img src="https://raw.githubusercontent.com/saivenkatsumanth9-stack/saivenkatsumanth9-stack/main/last-thief.jpg" width="180" alt="Last Thief Standing"/>

<br/><br/>

![Unity](https://img.shields.io/badge/Unity-000000?style=for-the-badge&logo=unity&logoColor=white&labelColor=1a1a2e)
![C#](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=csharp&logoColor=white&labelColor=1a1a2e)
![2D](https://img.shields.io/badge/2D_Animation-D4AF37?style=for-the-badge&labelColor=1a1a2e)
![Physics](https://img.shields.io/badge/Game_Physics-AA771C?style=for-the-badge&labelColor=1a1a2e)

</div>

---

| Feature | Description |
|:--------|:------------|
| 🕹️ **Endless Runner** | Complete 2D game built from scratch in Unity |
| ⚔️ **Combat System** | Jump & shoot mechanics + intelligent enemy AI |
| 💎 **Reward System** | Coins, gems & power-ups with upgrades |
| 📈 **Dynamic Difficulty** | Distance-based scaling that never ends |

> 💡 *A fully shipped game — not just a tutorial project*

</td>
</tr>
</table>

</div>

---

<!-- ═══════════════════════ GITHUB STATS DASHBOARD ═══════════════════════ -->

## <img src="https://media.giphy.com/media/cj87CxfRtrUifF3Ryk/giphy.gif" width="28"/> &nbsp;GitHub Stats Dashboard

<div align="center">

<!-- ═══ ROW 1: Stats + Streak ═══ -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://github-readme-stats.vercel.app/api?username=saivenkatsumanth9-stack&show_icons=true&hide_border=true&count_private=true&include_all_commits=true&title_color=D4AF37&icon_color=FFD700&text_color=f0e6d2&bg_color=050505&rank_icon=github&show=reviews,discussions_started,prs_merged"/>
  <img width="49%" src="https://github-readme-stats.vercel.app/api?username=saivenkatsumanth9-stack&show_icons=true&hide_border=true&count_private=true&include_all_commits=true&title_color=D4AF37&icon_color=FFD700&text_color=f0e6d2&bg_color=050505&rank_icon=github&show=reviews,discussions_started,prs_merged"/>
</picture>
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://streak-stats.demolab.com?user=saivenkatsumanth9-stack&hide_border=true&background=050505&ring=D4AF37&fire=FFD700&currStreakLabel=FFD700&sideLabels=f0e6d2&dates=8b949e&currStreakNum=f0e6d2&sideNums=f0e6d2"/>
  <img width="49%" src="https://streak-stats.demolab.com?user=saivenkatsumanth9-stack&hide_border=true&background=050505&ring=D4AF37&fire=FFD700&currStreakLabel=FFD700&sideLabels=f0e6d2&dates=8b949e&currStreakNum=f0e6d2&sideNums=f0e6d2"/>
</picture>

<br/><br/>

<!-- ═══ ROW 2: Languages Donut ═══ -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://github-readme-stats.vercel.app/api/top-langs/?username=saivenkatsumanth9-stack&layout=donut&hide_border=true&bg_color=050505&title_color=D4AF37&text_color=f0e6d2&langs_count=8"/>
  <img width="42%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=saivenkatsumanth9-stack&layout=donut&hide_border=true&bg_color=050505&title_color=D4AF37&text_color=f0e6d2&langs_count=8"/>
</picture>

</div>

---

<!-- ═══════════════════════ TROPHIES ═══════════════════════ -->

## 🏆 GitHub Trophies

<div align="center">
<img src="https://github-profile-trophy.vercel.app/?username=saivenkatsumanth9-stack&theme=discord&no-frame=true&no-bg=true&row=1&column=7&margin-w=10" alt="Trophies"/>
</div>

---

<!-- ═══════════════════════ CONTRIBUTION GRAPH ═══════════════════════ -->

## 📈 Contribution Graph

<div align="center">
<img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=saivenkatsumanth9-stack&bg_color=050505&color=D4AF37&line=FFD700&point=AA771C&area=true&area_color=1f190a&hide_border=true&custom_title=⚡%20Sumanth's%20Contribution%20Activity" alt="Contribution Graph"/>
</div>

---

<!-- ═══════════════════════ SNAKE ANIMATION ═══════════════════════ -->

## 🐍 Watch the Snake Eat My Contributions

<div align="center">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/saivenkatsumanth9-stack/saivenkatsumanth9-stack/output/github-snake-dark.svg"/>
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/saivenkatsumanth9-stack/saivenkatsumanth9-stack/output/github-snake.svg"/>
  <img alt="Snake animation" src="https://raw.githubusercontent.com/saivenkatsumanth9-stack/saivenkatsumanth9-stack/output/github-snake-dark.svg" width="100%"/>
</picture>

<br/>

<sub>🔧 <i>Auto-generated every 12 hours via GitHub Actions</i></sub>
</div>

---

<!-- ═══════════════════════ 3D CONTRIBUTION GRAPH ═══════════════════════ -->

## 🌐 3D Contribution Map

<div align="center">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="./profile-3d-contrib/profile-night-rainbow-animate.svg"/>
  <img alt="3D Contrib" src="./profile-3d-contrib/profile-green-animate.svg" width="100%"/>
</picture>

<br/>

<sub>🔧 <i>Auto-generated daily via GitHub Actions</i></sub>
</div>

---

<!-- ═══════════════════════ ACHIEVEMENTS TIMELINE ═══════════════════════ -->

## 🎯 Achievement Timeline

<div align="center">

```
  🚀 MY JOURNEY SO FAR
  ═══════════════════════════════════════════════════════════

  2023 ─────┐
            ├── 🎓 Started B.Tech CSE at NNRES
            ├── 💻 First lines of Python & C
            └── 🌱 Discovered the joy of programming
                │
  2024 ─────┤
            ├── ☕ Mastered Java & Object-Oriented Design
            ├── 🎮 Built "Last Thief Standing" in Unity/C#
            ├── 🤖 Dove deep into AI & Machine Learning
            └── 📊 Started exploring Data Analytics
                │
  2025 ─────┤
            ├── 🧠 Implemented RAG — "AI Study Buddy"
            ├── 🏋️ Shipped "HealthMate" — Full-Stack + AI
            ├── ⚛️  Began Quantum Computing exploration
            ├── 🌐 Learning React & Node.js ecosystem
            └── 🎯 Seeking internships & collaborations
                │
  2027 ─────┘
            └── 🎓 Graduation — Ready to change the world! 🌍
```

</div>

---

<!-- ═══════════════════════ ACHIEVEMENTS TABLE ═══════════════════════ -->

## 🏅 Milestones & Badges

<div align="center">

| &nbsp; | Achievement | Impact |
|:---:|:---|:---|
| 🤖 | Built **3 end-to-end projects** spanning Game Dev, Health Tech & AI/ML | Full-stack capability |
| 🧠 | Implemented **RAG** (Retrieval-Augmented Generation) at undergraduate level | Advanced AI skills |
| 🎮 | Shipped a **complete Unity 2D game** with enemy AI & difficulty scaling | Game dev mastery |
| 🎓 | Maintaining **7.45 CGPA** while building real-world projects | Balance & discipline |
| 🌐 | Built **responsive full-stack health app** with AI chatbot integration | Web dev + AI fusion |
| 🔍 | Actively **targeting software internships** in 3rd year | Career readiness |
| ⚛️ | Exploring **Quantum Computing** — staying ahead of the curve | Future-ready mindset |

</div>

---

<!-- ═══════════════════════ EDUCATION ═══════════════════════ -->

## 📚 Education

<div align="center">

| &nbsp; | Institution | Qualification | Year | Score |
|:---:|:---|:---|:---:|:---:|
| 🎓 | **Nalla Narasimha Reddy Education Society** | B.Tech – Computer Science & Engineering | 2023–2027 | CGPA: **7.45** |
| 📗 | **Sri Chaitanya Junior College** | 12th – Intermediate (MPC) | 2021–2023 | **81%** |
| 📘 | **Suprabath Model High School** | 10th – SSC | 2019–2021 | **69%** |

</div>

---

<!-- ═══════════════════════ INTERESTS ═══════════════════════ -->

## 💡 What Drives Me

<div align="center">

![Game Dev](https://img.shields.io/badge/🎮_Game_Dev-D4AF37?style=for-the-badge&logoColor=black&labelColor=000000)
![AI](https://img.shields.io/badge/🤖_AI_/_ML-D4AF37?style=for-the-badge&logoColor=black&labelColor=000000)
![Web Dev](https://img.shields.io/badge/🌐_Full_Stack-D4AF37?style=for-the-badge&logoColor=black&labelColor=000000)
![Data](https://img.shields.io/badge/📊_Data_Analytics-D4AF37?style=for-the-badge&logoColor=black&labelColor=000000)
![Quantum](https://img.shields.io/badge/⚛️_Quantum-D4AF37?style=for-the-badge&logoColor=black&labelColor=000000)
![Open Source](https://img.shields.io/badge/🔓_Open_Source-D4AF37?style=for-the-badge&logoColor=black&labelColor=000000)

<br/><br/>

**🌐 Languages I Speak:** &nbsp; 🇬🇧 English &nbsp;•&nbsp; 🇮🇳 Telugu &nbsp;•&nbsp; 🇮🇳 Hindi

</div>

---

<!-- ═══════════════════════ DYNAMIC WIDGETS ═══════════════════════ -->

## ⚡ Fun Extras

<div align="center">

<!-- ═══ RANDOM DEV QUOTE ═══ -->
<img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight&border=true" alt="Random Dev Quote"/>

<br/><br/>

<!-- ═══ RANDOM DEV JOKE ═══ -->
<img src="https://readme-jokes.vercel.app/api?theme=tokyonight&hideBorder&qColor=%23D4AF37&aColor=%23FFD700&bgColor=%23050505&borderColor=%23AA771C" alt="Random Dev Joke"/>

</div>

---

<!-- ═══════════════════════ CONNECT SECTION ═══════════════════════ -->

## 📬 Let's Build Together!

<div align="center">

<img src="https://media.giphy.com/media/LnQjpWaON8nhr21vNW/giphy.gif" width="60"/>

<br/><br/>

### 💼 Open to exciting **internships**, **projects** & **collaborations**!
### 🤝 Let's turn ideas into reality

<br/>

<a href="mailto:saivenkatsumanth9@gmail.com">
  <img src="https://img.shields.io/badge/📧_saivenkatsumanth9@gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/>
</a>
<br/><br/>
<a href="https://linkedin.com/in/saivenkatsumanth">
  <img src="https://img.shields.io/badge/🔗_Connect_on_LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>
<br/><br/>
<a href="https://github.com/saivenkatsumanth9-stack">
  <img src="https://img.shields.io/badge/⭐_Follow_on_GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
</a>

</div>

---

<!-- ═══ CUSTOM WAVE FOOTER DIVIDER ═══ -->
<img src="./assets/wave-footer.svg" width="100%" alt="wave"/>

<!-- ═══════════════════════ FOOTER ═══════════════════════ -->

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:000000,50:111111,100:D4AF37&height=150&section=footer&text=Thanks%20for%20visiting!%20⭐&fontSize=28&fontColor=ffffff&animation=twinkling&fontAlignY=65&desc=Drop%20a%20⭐%20if%20you%20found%20my%20work%20interesting!&descSize=14&descColor=FFD700&descAlignY=85" width="100%"/>

<div align="center">

<br/>

*"Code is like humor. When you have to explain it, it's bad."* — **Cory House**

<br/>

**Made with ❤️ and mass amounts of ☕ by [Sumanth](https://github.com/saivenkatsumanth9-stack)**

<br/>

<img src="https://img.shields.io/badge/Built_with-❤️_&_☕-D4AF37?style=for-the-badge&labelColor=000000" alt="Built with love"/>

</div>

<!-- ═══ END OF README ═══ -->
<!-- Last Updated: June 2025 -->
<!-- GitHub: @saivenkatsumanth9-stack -->
